...
또한 부모-자식 테이블로 분리된 설계 데이터 구조에서

데이터 누락으로 조회 값이 정상적으로 출력되지 않는 문제를 확인하고
...
이를 통해 서비스 단에서 별도의 분기 없이 동일한 방식으로 데이터를 처리할 수 있도록 구현했습니다.
...
설계 데이터의 진행상태는 만기일을 기준으로 변경되도록 구성했고,

스케줄러를 통해 자정 기준으로 상태가 자동 갱신되도록 했습니다.

배치 실행 시점과 사용자 확인 시점 사이의 시간 차를 고려해

필요한 경우 수동으로 상태를 반영할 수 있는 기능을 제공했습니다.
