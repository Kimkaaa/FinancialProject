...
<details>
  ...
  <br>
  <img src="./images/requirements_spec.png" alt="function_structure" width="600" />
</details>
...
요구사항 정의서와 테이블 정의서 등을 통해 기능 구조와 데이터 흐름을 정리했고, 이를 작업 기준으로 활용했습니다.

다만 작성 이후 변경 사항을 지속적으로 반영하고 공유하는 부분에서는 아쉬움이 있었고,
...
입력값 검증을 통해 null을 반환하도록 수정하고 서비스 계층에서 "N/A"와 실제 0 값을 구분해 처리했습니다.

<br>

또한 부모-자식 테이블로 분리된 설계 데이터 구조에서 데이터 누락으로

조회 값이 정상적으로 출력되지 않는 문제를 확인하고 검증 쿼리를 통해 원인을 파악해 데이터 정합성을 개선했습니다.

<br>

상품 설계 데이터는 상품 유형에 따라 서로 다른 테이블에 저장되어 있었기 때문에

조회 시 CASE문과 서브쿼리를 활용해 하나의 조회로 처리했습니다.

이를 통해 서비스 단에서는 별도 분기 없이 공통된 방식으로 다룰 수 있었습니다.

<br>

다만 반복되는 서브쿼리 구조로 인해 쿼리 복잡도가 높아지는 측면이 있었으며

데이터 규모가 증가할 경우 조인 기반으로 조회 구조를 개선하거나

검색 및 정렬에 사용되는 컬럼에 인덱스를 추가하는 방식으로 보완할 수 있습니다.

<br>

설계 데이터의 진행상태는 만기일을 기준으로 변경되도록 구성했고, 자정마다 스케줄러를 통해 갱신되도록 했습니다.

배치 실행 시점과 사용자 확인 시점 사이의 시간 차를 고려해 필요한 경우 수동으로 상태를 반영할 수 있는 기능을 제공했습니다.

<br>

Figma 기반으로 UI를 설계하고 공통 스타일을 적용해 화면의 일관성을 유지했으며,

검색 UX 개선과 버튼 기반 입력 등 사용자 편의성을 고려한 UI를 보완했습니다.

<br>

이번 프로젝트를 통해 설계와 구현, UI 개선까지 전반적인 과정을 경험할 수 있었고,

특히 커뮤니케이션이 작업의 효율과 완성도에 영향을 미친다는 점을 체감할 수 있었습니다.
